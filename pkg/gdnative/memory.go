...
func AllocArray[T any](len uint64) *[math.MaxUint32]T {
	var t T

	bytes := (uint64)(unsafe.Sizeof(t)) * len

	return (*[math.MaxUint32]T)(AllocZeros(bytes))
}
...
func AllocGDNativeTypePtrArray(size int) *[1<<30 - 1]GDNativeTypePtr {
	m := AllocZeros(uint64(uintptr(size) * sizeOfPtr))

	return (*[1<<30 - 1]GDNativeTypePtr)(m)
}
...
func VariantPtrSliceToGDNativeVariantPtrArray(p_args []*Variant) *[1<<30 - 1]GDNativeVariantPtr {
	gdnVariantPtrArgs := (*[1<<30 - 1]GDNativeVariantPtr)(AllocNewArrayAsUnsafePointer(p_args))
