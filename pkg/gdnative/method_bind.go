...
func (b *MethodBind) Call(
	...
	if (uint32)(p_argument_count) > b.ArgumentCount {
		r_error.error = (C.GDNativeCallErrorType)(GDNATIVE_CALL_ERROR_TOO_MANY_ARGUMENTS)
		r_error.argument = (C.int)(p_argument_count)
		return Variant{}
	}
...
func (b *MethodBind) Ptrcall(
	...
	inst := GDClassFromGDExtensionClassInstancePtr(ci, (C.GDExtensionClassInstancePtr)(p_instance))
...
func GoCallback_MethodBindBindGetArgumentType(p_method_userdata unsafe.Pointer, p_argument int32) GDNativeVariantType {
	bind := (*MethodBind)(p_method_userdata)
	return bind.ArgumentTypes[p_argument+1]
}

//export GoCallback_MethodBindBindGetArgumentInfo
func GoCallback_MethodBindBindGetArgumentInfo(p_method_userdata unsafe.Pointer, p_argument int32, r_info *GDNativePropertyInfo) {
	bind := (*MethodBind)(p_method_userdata)
	*r_info = bind.ArgumentPropertyInfos[p_argument+1]
}

//export GoCallback_MethodBindBindGetArgumentMetadata
func GoCallback_MethodBindBindGetArgumentMetadata(p_method_userdata unsafe.Pointer, p_argument int32) GDNativeExtensionClassMethodArgumentMetadata {
	bind := (*MethodBind)(p_method_userdata)
	return bind.ArgumentMetadatas[p_argument+1]
}

// this is called when methods in Go callback into GDScript
//
//export GoCallback_MethodBindBindCall
func GoCallback_MethodBindBindCall(p_method_userdata unsafe.Pointer, p_instance C.GDExtensionClassInstancePtr, p_args *GDNativeVariantPtr, p_argument_count GDNativeInt, r_return GDNativeVariantPtr, r_error *GDNativeCallError) {
	bind := (*MethodBind)(p_method_userdata)

	ci := bind.GetClassInfo()

	// TODO: what is the true arg count limit?
	...
	if (uint32)(p_argument_count) > bind.ArgumentCount {
		r_error.error = (C.GDNativeCallErrorType)(GDNATIVE_CALL_ERROR_TOO_MANY_ARGUMENTS)
		return
	}

	// add an extra parameter for the receiver instance
	args := make([]reflect.Value, bind.ArgumentCount+1)

	inst := GDClassFromGDExtensionClassInstancePtr(ci, p_instance)

	args[0] = reflect.ValueOf(inst)

	for i := 0; i < (int)(bind.ArgumentCount); i++ {
		var varg *Variant
		if i < (int)(p_argument_count) {
			varg = vArgs[i]
		} else {
			varg = bind.DefaultArguments[i]
		}

		args[i+1] = varg.ToReflectValue(
			bind.ArgumentTypes[i+1],
			bind.ArgumentGoTypes[i+1],
		)
	}

	log.Debug("argument converted",
		zap.String("method_name", (string)(bind.GoName)),
		zapGDNativeVariantPtrp("vargs", p_args, (int)(p_argument_count)),
		zap.String("args", spew.Sdump(args)),
	)

	reflectedRet := bind.Func.Call(args)
...
func GoCallback_MethodBindBindPtrcall(p_method_userdata unsafe.Pointer, p_instance GDExtensionClassInstancePtr, p_args *GDNativeTypePtr, r_ret GDNativeTypePtr) {
	bind := (*MethodBind)(p_method_userdata)
	bind.Ptrcall(p_instance, p_args, &r_ret)
}

// func call_with_variant_args_retc_dv(p_instance *GDExtensionClassInstancePtr, p_method unsafe.Pointer, p_args []GDNativeVariantPtr, methodArgCount int32, default_values []GDNativeVariantPtr) (*Variant, error) {

// 	if len(p_args) > methodArgCount {
// 		r_error = &GDNativeCallError{}
// 		r_error.error = GDNATIVE_CALL_ERROR_TOO_MANY_ARGUMENTS
// 		r_error.argument = methodArgCount
// 		return nil, err
// 	}

// 	// count missing arg count
// 	missingArgCount := methodArgCount - len(p_args)

// 	dvs := len(default_values)
...
// 	// reminder: Avoid zero sized array.
// 	var args []*Variant

// 	for i := 0; i < methodArgCount; i++ {
// 		if i < len(p_args) {
// 			args[i] = (*Variant)(p_args[i])
// 		} else {
// 			args[i] = (*Variant)(default_values[i-len(p_args)+(dvs-missing)])
// 		}
// 		args = append(args, p_args[i])
// 	}
...
// func call_with_variant_args_retc_helper(p_instance *GDExtensionClassInstancePtr, p_method unsafe.Pointer, const Variant **p_args, Variant &r_ret, GDNativeCallError &r_error, IndexSequence<Is...>) {
