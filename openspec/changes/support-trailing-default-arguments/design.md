...
A third layer followed from the registration shape: with `argument_count = 0`, a nonzero `default_argument_count` makes the engine's parse-time check compute `required = 0 - defaults` as an unsigned underflow (e.g. "Expected at least 4294967295"), rendering every call to such a binding a parse error. Defaults are meaningless for pure varargs anyway — the fill is skipped and Godot has no varargs-with-defaults pattern. Decision: reject `isVariadic && len(defaultArguments) > 0` at bind time with a clear panic naming the method, alongside the existing D4 count guard.
