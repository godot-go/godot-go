...
type GDClass interface {
	...
	GetExtensionParentClass() TypeName
}
...
func gdClassRegisterInstanceBindingCallbacks(tn TypeName) {
	...
	// };
	var cbs C.GDNativeInstanceBindingCallbacks

	cbs.create_callback = (*[0]byte)(C.cgo_gdclass_binding_create_callback)
	cbs.free_callback = (*[0]byte)(C.cgo_gdclass_binding_free_callback)
	cbs.reference_callback = (*[0]byte)(C.cgo_gdclass_binding_reference_callback)
...
func GDClassFromGDExtensionClassInstancePtr(p_classinfo *ClassInfo, p_instance C.GDExtensionClassInstancePtr) GDClass {
	if p_instance == (C.GDExtensionClassInstancePtr)(nullptr) {
...
func GoCallback_GDNativeExtensionClassCreateInstance(data unsafe.Pointer) C.GDNativeObjectPtr {
	...
	inheritName := ci.InheritType.Name()

	log.Debug("GoCallback_GDNativeExtensionClassCreateInstance called", zap.String("class_name", (string)(tn)), zap.Any("inherit_name", inheritName))

	// create inherited GDNativeClass first
	owner := GDNativeInterface_classdb_construct_object(internal.gdnInterface, inheritName)
	...
	log.Debug("GDClass instance created",
		...
		zap.Any("inherit_name", inheritName))
...
func GoCallback_GDNativeExtensionClassFreeInstance(data unsafe.Pointer, ptr C.GDExtensionClassInstancePtr) {
	...
	// ptr is assigned in WrappedPostInitialize as a (*Wrapped)
