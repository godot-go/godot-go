...
- [x] 6.1 Add `TestMutateArray` (in-place `Append` on the received argument) and `TestCloneEchoArray`/`TestRebuildArrayPlus` to `test/pkg/example.go`, register via `ClassDBBindMethod`; assert in `test/demo/main.gd`: Array mutation propagates to the caller in both call styles (`Array` is shared-reference, no CoW), a byte-identical defensive clone round-trips as a borrow echo, and a rebuilt array (bytes differing from every argument) round-trips without mutating or leaking
- [x] 6.2 Sensitivity-verify the new tests: temporarily reverting the ptrcall `PackedInt64Array` decode to an owned copy fails the propagation assertion; temporarily disabling `isPtrcallBorrowEcho` produces `_ref` refcount-underflow errors and a crash on echo round-trips; both temporary mutations reverted afterwards
- [x] 6.3 Run `GODOT=/path/to/godot make build` and `make test` — suite green (646 passes, 0 failures)
- [x] 6.4 Post-review rework: mutating a *borrowed packed array* is undefined behavior (the borrow holds no `CowData` refcount, so reallocating mutations free the caller's buffer — intermittent CI double-free); remove `TestMutatePackedInt64Array` and its assertions, narrow the spec to read-only packed borrows + shared-reference Array/Dictionary propagation, update design.md/proposal.md accordingly
