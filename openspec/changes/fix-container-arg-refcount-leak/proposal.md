...
- Behavior/contract change: a Go method receives container arguments as borrows valid only for the duration of the call; storing one past the call requires an explicit copy. This matches the existing StringName/NodePath contract. The container decode paths are new and unreleased, so no existing callers are broken.
