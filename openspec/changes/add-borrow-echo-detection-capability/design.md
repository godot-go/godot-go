...
Covered types today: `StringName`, `NodePath`, `Array`, and nine `Packed*Array` types — exactly those whose **ptrcall** arguments decode as borrows. Their varcall argument decodes differ per type (`StringName`/`NodePath` decode as borrows; containers decode as owned copies released post-call by `destroyOwnedContainerArgs`), and source-release behavior on return varies by type × path: ptrcall releases all covered types; the varcall encoder releases only `StringName`/`NodePath` — container returns via varcall get no return-path source release, a pre-existing behavior deliberately deferred in the container change's Non-Goals and now recorded normatively here rather than described as correct.
