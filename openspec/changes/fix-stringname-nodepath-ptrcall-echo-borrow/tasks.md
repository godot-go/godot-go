...
- [x] 1.1 In `isPtrcallBorrowEcho` (`pkg/core/method_bind.go`), add `StringName` and `NodePath` to the type switch so echoed borrows of these types are recognized
- [x] 1.2 In `GDExtensionTypePtrFromReflectValue` (`pkg/core/variant_refect_value.go`), gate the `StringName`/`NodePath` `inst.Destroy()` calls on `destroySource`, matching the container cases
- [x] 1.3 Add a `destroySource bool` parameter to `GDExtensionVariantPtrFromReflectValue` (mirroring its type-ptr counterpart), gate the `StringName`/`NodePath` destroys on it, and update both call sites in `GoMethodMetadata.Call`: the standard branch computes the flag via `isPtrcallBorrowEcho(ret[0], args)`, the variadic branch always passes true (it never holds borrows)
- [x] 1.4 Remove the stale post-call StringName/NodePath arg-destroy loop from `GoMethodMetadata.Call` (design Decision 3 — it double-releases borrow-decoded args)
- [x] 1.5 Verify `go build ./pkg/core/...` compiles
...
- [x] 3.1 Run `go vet ./pkg/core/...` on changed files
- [x] 3.2 Run `GODOT=/path/to/godot make build` and `make test`; suite green with no orphan StringName/NodePath warnings in output
