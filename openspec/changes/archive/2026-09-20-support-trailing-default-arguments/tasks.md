...
- [x] 8.5 In `NewGDExtensionClassMethodInfoFromMethodBind` (`pkg/core/method_bind.go`), register pure-varargs bindings with `argument_count = 0` (no named `argument_info` entries) plus the vararg flag, so GDScript parse-time accepts zero-argument varargs calls (the engine counts each registered argument as required; vararg only relaxes too-many — cf. native vararg binds like `Object.call` registering only leading named args); the varcall callback receives raw args regardless
- [x] 8.6 `GODOT=/path/to/godot make build` and `make test` green with the zero-arg `varargs_func()` demo assertion
- [x] 8.7 In `NewGoMethodMetadata` (`pkg/core/method_bind.go`), reject `isVariadicFlaged && len(defaultArguments) > 0` at bind time with a panic naming the method (with `argument_count = 0` registered, a nonzero default count underflows the engine's parse-time required-count computation; defaults are meaningless for pure varargs since the fill is skipped)
- [x] 8.8 Retarget `TestFillCallArgsVariadicSkipsFill` to the real shape (`defaults = 0`) so removing the skip trips the unfilled-slot panic directly; add `TestBindRejectsVariadicDefaults` asserting the new bind-time rejection; remove the now-dead `gdeVarArgProp*` lifecycle fields (struct, constructor creation, `Destroy` block) that no registration references
- [x] 8.9 `go vet ./pkg/... ./test/pkg/...` clean, `go test ./pkg/core/` green, and `GODOT=/path/to/godot make build && make test` green
